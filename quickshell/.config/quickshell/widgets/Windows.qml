...
ScriptWidget {
  ...
  // we pick the token.
  readonly property color focusColor: hasClass("scratchpad") ? Theme.attention : Theme.accent
  ...
  function rebuild() {
    var titles = parseSpans(root.text)
    var fulls = parseSpans(root.scriptTooltip)
  ...
  readonly property string listedWorkspace: {
    if (hasClass("scratchpad")) return "special:magic"
  ...
  function syncGlow() {
    for (var i = 0; i < repeater.count; i++) {
      ...
      if (entry && entry.isFocused && entry.labelItem) {
  ...
  Item {
    ...
    Row {
      ...
      // visibly fill and empty on every switch. Plain quick fade instead.
      add: Transition {
      ...
      Repeater {
        ...
        delegate: Row {
          ...
          // waybar's rather than approximating it with a Row spacing.
          Text {
            text: "  │  "
